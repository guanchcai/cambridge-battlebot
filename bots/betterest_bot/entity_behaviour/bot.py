...
class Bot(EBase):
    ...
    def update_map(self):
        ...
        for t in self.ct.get_nearby_tiles():
            ...
            bot_team = self.ct.get_team(bot_id) == self.team if bot_id else None
            ...
            if (
                not (tile.passable(self.ct) or tile.bot_id == self.id or tile.destroyable()) and 
                self.distance_map and 
                t in self.distance_map and 
                t != self.current_target_position
            ):
                print(f"Encountered wall at {t}")
                self.encountered_wall(t)
    ...
    def move_to_pos(self):
        ...
        already_reached = dist_to_target <= self.target_distance_squared
        ...
        if self.distance_map is None:
            print(f"Can't reach {self.current_target_position} from {self.position}")
        ...
        if not self.distance_map:
            # Handle already_reached in next tick
            return
        ...
        print(f"Build road finish {self.ct.get_cpu_time_elapsed()}, success: {build_success}")
        ...
        if self.ct.get_position() != self.position:

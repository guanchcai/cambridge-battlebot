...
class Aggressor(Bot):
    ...
    def reached_target(self):        
        ...
        if (target_data.building_type in IGNORED_BUILDINGS or target_data.is_team_road()):
            if can_build:
                ...
                if self.current_target_source and direction == self.current_target_position.direction_to(self.current_target_source):
                    rotate_left_pos = self.current_target_position.add(direction.rotate_left())
                    rotate_right_pos = self.current_target_position.add(direction.rotate_right())
                    enemy_base_pos = self.get_enemy_base()
                    if rotate_left_pos.distance_squared(enemy_base_pos) <= rotate_right_pos.distance_squared(enemy_base_pos):
                ...
                self.current_target_type == TargetTypes.AGG_HARVESTER and
                self.check_for_entity(self.current_target_source, CARDINAL_DIRECTIONS, EntityType.SENTINEL, self.team) and 
                self.ct.get_global_resources()[0] < self.ct.get_sentinel_cost()[0]
            ):
                if self.position == self.current_target_position:
                    self.move_to_adjacent()
    ...
    def evaluate_aggressor_target(self, tile: Position, tile_data: TileData):
        ...
        def evaluate_conveyors():
            ...
            eval -= sum([tile.distance_squared(enemy_bot_pos) <= 2 for enemy_bot_pos in self.enemy_bots]) * 5
        ...
        if tile_data and tile_data.building_type == EntityType.HARVESTER:
            evaluate_harvesters()
        elif self.enemy_base_pos and tile_data.building_type in CONVEYORS:
            evaluate_conveyors()
        ...
        if etype in IGNORED_BUILDINGS or etype == EntityType.ROAD:
            ...
            for d in CARDINAL_DIRECTIONS:
                ...
                if check_data and check_data.building_type == EntityType.HARVESTER:
                    ...
                    return True
            for d in BRIDGE_DELTAS + CARDINAL_DELTAS:
